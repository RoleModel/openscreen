...
// Also the brand boards under /wallpapers/brand/, which is where this fork's
// bundled wallpapers live (upstream only ever had the numbered stock set, and
// this fork deleted that). Still a bundled, relative, extension-pinned path —
// no traversal, no file:// and no URL — so the self-contained rule holds.
const BUNDLED_WALLPAPER_RE = /^\/wallpapers\/(?:wallpaper\d+|brand\/[a-z0-9-]+)\.jpg$/;
