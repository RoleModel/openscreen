/*
 * The default backdrop, written out rather than imported.
 *
 * This module is shared by the renderer AND the electron main/preload bundles,
 * and those are built by vite-plugin-electron with their own configs — which
 * carry no `@` alias. Importing `@/lib/wallpaper` here therefore typechecks,
 * passes every test, and breaks `vite build` at the electron pass with
 * "Rollup failed to resolve import". It would also drag brandWallpapers and
 * assetPath into the main process, which have no business there.
 *
 * It is the first entry of BRAND_WALLPAPERS in src/lib/brandWallpapers.ts,
 * which is generated by the rolemodel-openscreen toolkit — the stock gradient
 * this used to name was deleted with the rest of them.
 */
const DEFAULT_WALLPAPER = "/wallpapers/brand/rm-dark-dotgrid.jpg";
