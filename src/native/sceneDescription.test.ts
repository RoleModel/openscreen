...
describe("buildSceneDescription.webcamEffect", () => {
	...
	it("carries the mode and its parameters, never pixels", () => {
		...
		expect(buildSceneDescription(doc).webcamEffect).toEqual({
			mode: "blur",
			blurIntensity: 0.85,
			// The default webcam wallpaper, which this fork points at a brand board —
			// the stock gradient upstream names here does not exist in this build.
			background: { kind: "image", path: DEFAULT_WALLPAPER },
		});
	});
