...
export class DocumentService {
	...
	async addAsset(projectId: string, input: AddAssetInput): Promise<AxcutDocument> {
		...
		// Upstream lets the caller declare the kind; this fork derives it from the
		// extension, because the Studio's drop path hands over a bare path. Take the
		// declaration when there is one and fall back to the guess, so neither
		// caller regresses.
		const kind = input.kind ?? assetKindForPath(input.path) ?? "video";
		if (kind === "audio") {
