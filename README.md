...
> - **The editor inside that window**, as a `WebContentsView` rather than a second
>   window, so the Studio's navigation stays beside it — `electron/studio/embedded-editor.ts`.
> - **RoleModel brand tokens** in `src/styles/`, mapped onto Optics rather than copied,
>   plus DM Sans and our app icon.
> - **The brand wallpapers in the editor's picker**, ahead of the stock set —
>   `src/lib/brandWallpapers.ts`, generated by the toolkit.
> - **`npm run app`** to launch from a checkout without the environment incantation.
> - Portable cargo discovery.
>
> Named **RoleModel Studio** everywhere a person reads, while the bundle on disk stays
> `Openscreen.app` — that name is a contract with the cask's `app` stanza, the
> `openscreen` shim and the DMG. From **0.0.1** the bundle id is `rolemodel.studio`
> rather than upstream's, which makes this a separate app rather than one installed
> over theirs. macOS keys the Screen Recording grant to that id, so a machine that had
> granted it to an earlier build is asked once more.
>
